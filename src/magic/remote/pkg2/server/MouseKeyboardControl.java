...
public class MouseKeyboardControl {
    ...
    public void resetRobot() {
        try {
            ...
            Logger.getLogger(MouseKeyboardControl.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void mouseMove(double start_x, double start_y, double end_x, double end_y) {
        
        double diffX = end_x-start_x;
        double diffY = end_y-start_y;
        int steps = (int) Math.round(Math.min(Math.max(diffX, diffY), 20));
        for (int i = 1; i <= steps; i++) {
            int mov_x = (int) (start_x + diffX*i/steps);
            int mov_y = (int) (start_y + diffY*i/steps);
            robot.mouseMove(mov_x, mov_y);
        }
        robot.mouseMove((int)end_x, (int)end_y);
    }
    ...
    public void typeCharacter(char character) {
        switch (character) {
            ...
            case '\b':
                ...
                break;
            default:
                System.out.println("<[" + character + "]> is not supported yet ");
